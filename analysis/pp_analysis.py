...
def fit_truncated_power_law(sizes: np.ndarray, s_min: int = 2) -> Dict:
    ...
    try:
        ...
        return {"tau": popt[0], "tau_se": perr[0], "s_c": popt[1], "valid": True, "n": len(sizes)}
    except Exception:
        return {"tau": np.nan, "s_c": np.nan, "valid": False, "n": len(sizes)}
...
def run_single_simulation(
    ...
    if with_evolution and evolved_vals:
        ...
        result["evolved_death_mean"] = float(np.mean(valid_evolved)) if valid_evolved else np.nan
        result["evolved_death_std"] = float(np.std(valid_evolved)) if valid_evolved else np.nan
    ...
    for pb in prey_births:
        for pd in pred_deaths:
            for rep in range(cfg.n_replicates):
                ...
                jobs.append((pb, pd, cfg.default_grid, seed_base, False))  # No evolution
                jobs.append((pb, pd, cfg.default_grid, seed_base, True))   # With evolution
...
def generate_plots(cfg: Config, output_dir: Path, logger: logging.Logger):
    ...
    for r in results:
        key = (round(r["prey_birth"], 4), round(r["pred_death"], 4), r["with_evolution"])
    ...
    for i, pd in enumerate(pred_deaths):
        for j, pb in enumerate(prey_births):
            ...
            if no_evo:
                ...
                grids["survival_no_evo"][i, j] = np.mean([r["pred_survived"] for r in no_evo]) * 100
            ...
            if evo:
                ...
                grids["survival_evo"][i, j] = np.mean([r["pred_survived"] for r in evo]) * 100
    ...
    ax = axes[0, 0]
    im = ax.imshow(grids["pred_pop_no_evo"], origin='lower', aspect='auto', extent=extent, cmap='YlOrRd')
    ax.contour(prey_births, pred_deaths, grids["survival_no_evo"], levels=[50], colors='black', linewidths=2)
    ...
    ax = axes[0, 1]
    im = ax.imshow(grids["pred_pop_evo"], origin='lower', aspect='auto', extent=extent, cmap='YlOrRd')
    ax.contour(prey_births, pred_deaths, grids["survival_evo"], levels=[50], colors='black', linewidths=2)
    plt.colorbar(im, ax=ax, label='Population')
    ax.set_xlabel('Prey Birth Rate')
    ax.set_ylabel('Predator Death Rate')
    ...
    advantage = np.where(
        ...
        (grids["pred_pop_evo"] - grids["pred_pop_no_evo"]) / grids["pred_pop_no_evo"] * 100,
        np.where(grids["pred_pop_evo"] > 10, 500, 0)
    )
    im = ax.imshow(np.clip(advantage, -50, 200), origin='lower', aspect='auto', extent=extent, cmap='RdYlGn', vmin=-50, vmax=200)
    ...
    ax = axes[1, 0]
    im = ax.imshow(grids["tau_prey"], origin='lower', aspect='auto', extent=extent, cmap='coolwarm', vmin=1.5, vmax=2.5)
    ax.contour(prey_births, pred_deaths, grids["tau_prey"], levels=[2.05], colors='green', linewidths=2)
    ...
    ax = axes[1, 1]
    im = ax.imshow(grids["tau_pred"], origin='lower', aspect='auto', extent=extent, cmap='coolwarm', vmin=1.5, vmax=2.5)
    ax.contour(prey_births, pred_deaths, grids["tau_pred"], levels=[2.05], colors='green', linewidths=2)
    plt.colorbar(im, ax=ax, label='τ')
    ax.set_xlabel('Prey Birth Rate')
    ax.set_ylabel('Predator Death Rate')
    ...
    ax = axes[1, 2]
    im = ax.imshow(dP_dd_evo, origin='lower', aspect='auto', extent=extent, cmap='RdBu_r', vmin=-5000, vmax=5000)
    ax.contour(prey_births, pred_deaths, dP_dd_evo, levels=[0], colors='black', linewidths=2)
    ...
    if sens_file.exists():
        ...
        ax.errorbar(sd_vals, pops, yerr=stds, fmt='o-', capsize=5, markersize=8, linewidth=2)
        ax.set_xlabel('Evolution SD', fontsize=12)
        ax.set_ylabel('Mean Predator Population', fontsize=12)
        ax.set_title('Evolution Sensitivity Analysis\n(prey_birth=0.30, pred_death=0.14)', fontsize=14)
    ...
    if fss_file.exists():
        ...
        if len(valid) >= 2:
            ...
            ax.plot(L_line, np.exp(intercept) * L_line**slope, 'g--', label=f'Fit: $s_c \\sim L^{{{slope:.2f}}}$')
            ax.plot(L_line, (L_line/100)**1.896 * 1000, 'k:', alpha=0.5, label='Theory: $L^{1.90}$')
        ax.set_xscale('log')
        ax.set_yscale('log')
        ax.set_xlabel('Grid Size L')
    ...
    summary = {
        ...
        "hydra_region_size": int(np.sum((dP_dd_evo > 0) & (grids["pred_pop_evo"] > 50))),
        "max_hydra_strength": float(np.nanmax(dP_dd_evo)),
    }
    
    # Find critical point
    dist_crit = np.sqrt((grids["tau_prey"] - 2.05)**2 + (grids["tau_pred"] - 2.05)**2)
    ...
    logger.info(f"Hydra effect region (dP/dd > 0): {summary['hydra_region_size']} combinations")
    if "critical_prey_birth" in summary:
        ...
        logger.info(f"  τ_prey = {summary['critical_tau_prey']:.3f}, τ_pred = {summary['critical_tau_pred']:.3f}")
...
def main():
    ...
    parser.add_argument("--cores", type=int, default=-1,
                        help="Number of cores (-1 = all)")
    parser.add_argument("--dry-run", action="store_true",
                        help="Print estimated runtime and exit")
