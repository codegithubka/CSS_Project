...
def load_phase4_results(
    results_dir: Path, filename: str = "phase4_results.jsonl"
) -> List[Dict]:
    """
    ...
    if jsonl_file.exists():
        ...
        raise FileNotFoundError(
            f"Phase 4 results not found: {jsonl_file} or {json_file}"
        )
...
def extract_parameter_grid(
    ...
    if config and "prey_birth_min" in config:
        prey_births = np.linspace(
            config["prey_birth_min"], config["prey_birth_max"], config["n_prey_birth"]
        )
        prey_deaths = np.linspace(
            config["prey_death_min"], config["prey_death_max"], config["n_prey_death"]
        )
    else:
        # Infer from data
...
def aggregate_to_grids(
    results: List[Dict], prey_births: np.ndarray, prey_deaths: np.ndarray
) -> Dict[str, np.ndarray]:
    ...
    for i, pd in enumerate(prey_deaths):
        for j, pb in enumerate(prey_births):
            ...
            if no_evo:
                grids["prey_pop_no_evo"][i, j] = np.mean(
                    [r["prey_mean"] for r in no_evo]
                )
...
def compute_hydra_derivative(
    grids: Dict[str, np.ndarray], prey_deaths: np.ndarray
) -> Tuple[np.ndarray, np.ndarray]:
...
def plot_phase_diagrams(
    grids: Dict,
    prey_births: np.ndarray,
    prey_deaths: np.ndarray,
    dN_dd_no_evo: np.ndarray,
    output_dir: Path,
    dpi: int = 150,
):
    ...
    ax = axes[0, 1]
    im = ax.imshow(
        grids["prey_pop_evo"], origin="lower", aspect="auto", extent=extent, cmap="YlGn"
    )
    ax.contour(
        prey_births,
        prey_deaths,
...
def plot_hydra_analysis(
    grids: Dict,
    prey_births: np.ndarray,
    prey_deaths: np.ndarray,
    dN_dd_no_evo: np.ndarray,
    dN_dd_evo: np.ndarray,
    output_dir: Path,
    dpi: int = 150,
):
...
def plot_fss_analysis(fss_results: List[Dict], output_dir: Path, dpi: int = 150):
    ...
    for L in sizes:
        tau_vals = [
            r["prey_tau"] for r in by_size[L] if not np.isnan(r.get("prey_tau", np.nan))
        ]
        if tau_vals:
    ...
    for L in sizes:
        sc_vals = [
            r["prey_s_c"] for r in by_size[L] if not np.isnan(r.get("prey_s_c", np.nan))
        ]
        if sc_vals:
    ...
    if np.sum(valid) >= 2:
        ...
        ax.plot(
            sizes,
            np.exp(intercept) * np.array(sizes) ** slope,
            "r--",
...
def plot_sensitivity_analysis(
    sens_results: List[Dict], output_dir: Path, dpi: int = 150
):
    ...
    ax.errorbar(
        sd_values, trait_means, yerr=trait_ses, fmt="o-", capsize=5, linewidth=2
    )
    ax.set_xlabel("Mutation Strength σ")
    ...
    for sd in sd_values:
        tau_vals = [
            r["prey_tau"] for r in by_sd[sd] if not np.isnan(r.get("prey_tau", np.nan))
        ]
        if tau_vals:
...
def plot_phase2_soc_analysis(
    ...
    fig.suptitle(
        "Phase 2: Self-Organized Criticality Test", fontsize=14, fontweight="bold"
    )
    ...
    ax.scatter(
        initial_pd, final_pd, alpha=0.4, s=30, c="steelblue", label="Individual runs"
    )
    ...
    scatter = ax.scatter(
        final_pd, prey_pops, c=initial_pd, cmap="viridis", alpha=0.6, s=40
    )
    ...
    summary = {
        ...
        "n_replicates_per_condition": (
            len(final_pd) // len(unique_initial) if len(unique_initial) > 0 else 0
        ),
        ...
        "distance_from_critical": float(
            np.abs(np.mean(final_pd) - critical_prey_death)
        ),
        "convergence_achieved": float(np.std(final_pd))
        < 0.02,  # Low variance = convergence
    }
    ...
    logging.info(
        f"  Final prey_death mean: {summary['final_prey_death_mean']:.4f} ± {summary['final_prey_death_std']:.4f}"
    )
...
def plot_phase3_fss_analysis(results: List[Dict], output_dir: Path, dpi: int = 150):
    ...
    for r in results:
        ...
        if r.get("prey_largest_fraction") is not None and not np.isnan(
            r.get("prey_largest_fraction", np.nan)
        ):
    ...
    fig, axes = plt.subplots(2, 2, figsize=(14, 12))
    fig.suptitle(
        "Phase 3: Finite-Size Scaling at Critical Point", fontsize=14, fontweight="bold"
    )
    ...
    if len(L_vals_frac) >= 3:
        ...
        fit_L = np.logspace(
            np.log10(L_vals_frac.min()), np.log10(L_vals_frac.max()), 50
        )
    ...
    summary = {
        ...
        "n_replicates_per_size": (
            len(results) // len(grid_sizes) if len(grid_sizes) > 0 else 0
        ),
    }
    ...
    for L in grid_sizes:
        summary["per_size"][f"L_{L}"] = {
            "s_max_mean": (
                float(np.mean(data_by_L[L]["prey_max_cluster"]))
                if data_by_L[L]["prey_max_cluster"]
                else None
            ),
            "s_max_std": (
                float(np.std(data_by_L[L]["prey_max_cluster"]))
                if data_by_L[L]["prey_max_cluster"]
                else None
            ),
            "largest_frac_mean": (
                float(np.mean(data_by_L[L]["prey_largest_frac"]))
                if data_by_L[L]["prey_largest_frac"]
                else None
            ),
            "n_clusters_mean": (
                float(np.mean(data_by_L[L]["prey_n_clusters"]))
                if data_by_L[L]["prey_n_clusters"]
                else None
            ),
        }
    ...
    if "s_max_exponent" in summary:
        logging.info(
            f"  s_max scaling exponent: {summary['s_max_exponent']:.3f} (R²={summary['s_max_exponent_r2']:.3f})"
        )
...
def plot_phase4_sensitivity_analysis(
    results: List[Dict],
    output_dir: Path,
    dpi: int = 150,
    critical_prey_death: float = 0.0963,
    ...
    has_evolution = (
        "evolved_prey_death_final" in df.columns
        and df["evolved_prey_death_final"].notna().any()
    )
    ...
    logging.info(
        f"  prey_birth values: {len(prey_births)} ({min(prey_births):.2f} to {max(prey_births):.2f})"
    )
    logging.info(
        f"  prey_death values: {len(prey_deaths)} ({min(prey_deaths):.2f} to {max(prey_deaths):.2f})"
    )
    logging.info(
        f"  pred_birth values: {len(pred_births)} ({min(pred_births):.2f} to {max(pred_births):.2f})"
    )
    logging.info(
        f"  pred_death values: {len(pred_deaths)} ({min(pred_deaths):.2f} to {max(pred_deaths):.2f})"
    )
    ...
    fig, axes = plt.subplots(2, 2, figsize=(14, 12))
    fig.suptitle(
        "Phase 4: Coexistence Regions Across Parameter Space",
        fontsize=14,
        fontweight="bold",
    )
    ...
    for i, pd_val in enumerate(prey_deaths):
        for j, pb_val in enumerate(prey_births):
            ...
            if len(subset) > 0:
                ...
                coex_rate = (
                    (subset["prey_survived"] == True)
                    & (subset["pred_survived"] == True)
                ).mean()
    ...
    for i, pred_d in enumerate(pred_deaths):
        for j, pred_b in enumerate(pred_births):
            subset = df[
                (df["predator_death"] == pred_d) & (df["predator_birth"] == pred_b)
            ]
            if len(subset) > 0:
                coex_rate = (
                    (subset["prey_survived"] == True)
    ...
    for pred_b in pred_births[::2]:
        ...
        for pd_val in prey_deaths[:-1]:
            ...
            subset_next = df[
                (df["predator_birth"] == pred_b)
                & (df["prey_death"] == prey_deaths[prey_deaths.index(pd_val) + 1])
            ]
        ...
        ax.plot(
            prey_deaths[:-1], hydra_by_pd, "o-", label=f"pred_b={pred_b:.1f}", alpha=0.7
        )
    ...
    if has_evolution:
        fig, axes = plt.subplots(2, 2, figsize=(14, 12))
        fig.suptitle(
            "Phase 4: Self-Organized Criticality Across Parameter Regimes",
            fontsize=14,
            fontweight="bold",
        )
        ...
        for pred_b in pred_births[::2]:
            ...
            if len(subset) > 0:
                ax.hist(
                    subset["evolved_prey_death_final"],
                    bins=15,
                    alpha=0.5,
                    label=f"pred_b={pred_b:.1f}",
                )
        ...
        ax = axes[1, 0]
        evolved_by_pred = df_evo.groupby(["predator_birth", "predator_death"])[
        ...
        grouped = (
            df_evo.groupby(["prey_birth", "predator_birth", "predator_death"])
            .agg({"evolved_prey_death_final": "mean", "prey_mean": "mean"})
            .reset_index()
        )

        ax.scatter(
            grouped["evolved_prey_death_final"],
            grouped["prey_mean"],
            alpha=0.5,
            s=30,
            c="steelblue",
    ...
    summary = {
        ...
        "coexistence": {
            "overall_rate": float(
                ((df["prey_survived"] == True) & (df["pred_survived"] == True)).mean()
                * 100
            ),
        ...
        "hydra_effect": {
            ...
            "mean_location": (
                float(np.mean(hydra_locations)) if hydra_locations else None
            ),
            "max_strength": (
                float(np.nanmax(hydra_grid)) if np.any(hydra_grid > 0) else 0
            ),
        },
    ...
    if has_evolution:
        summary["soc"] = {
            ...
            "distance_from_critical": float(
                abs(df_evo["evolved_prey_death_final"].mean() - critical_prey_death)
            ),
        }
    ...
    logging.info(
        f"  Overall coexistence rate: {summary['coexistence']['overall_rate']:.1f}%"
    )
    logging.info(f"  Hydra effect detected: {summary['hydra_effect']['detected']}")
    if summary["hydra_effect"]["mean_location"]:
        logging.info(
            f"  Mean hydra location: {summary['hydra_effect']['mean_location']:.4f}"
        )
    if has_evolution:
        logging.info(
            f"  Mean evolved prey_death: {summary['soc']['mean_evolved_prey_death']:.4f}"
        )
        logging.info(
            f"  Distance from critical: {summary['soc']['distance_from_critical']:.4f}"
        )
...
def generate_summary_report(
    ...
    summary = {
        ...
        "hydra_region_size": int(
            np.sum((dN_dd_no_evo > 0) & (grids["prey_pop_no_evo"] > 50))
        ),
...
Examples:
  ...
  %(prog)s results/ --fss-only         # Only FSS plots
    ...
    parser.add_argument(
        "results_dir", type=Path, help="Directory containing analysis results"
    )
    parser.add_argument(
        "--phase-only", action="store_true", help="Generate only phase diagrams"
    )
    parser.add_argument(
        "--hydra-only", action="store_true", help="Generate only Hydra analysis plots"
    )
    parser.add_argument(
        "--pcf-only", action="store_true", help="Generate only PCF analysis plots"
    )
    parser.add_argument(
        "--fss-only", action="store_true", help="Generate only FSS plots"
    )
    parser.add_argument(
        "--sensitivity-only",
        action="store_true",
    ...
    if plot_all or args.phase_only or args.hydra_only or args.pcf_only:
        try:
            ...
            if plot_all or args.phase_only:
                plot_phase_diagrams(
                    grids, prey_births, prey_deaths, dN_dd_no_evo, output_dir, args.dpi
                )
            ...
            if plot_all:
                summary = generate_summary_report(
                    grids, dN_dd_no_evo, prey_births, prey_deaths, output_dir
                )
                logging.info("SUMMARY:")
                logging.info(f"  Hydra region size: {summary['hydra_region_size']}")
                logging.info(
                    f"  Max Hydra strength: {summary['max_hydra_strength']:.1f}"
                )
                if "critical_prey_birth" in summary:
                    logging.info(
                        f"  Critical point: pb={summary['critical_prey_birth']:.3f}, "
    ...
    if plot_all or args.bifurcation_only:
        try:
            ...
            logging.info(
                f"Loaded bifurcation results: {len(sweep_params)} sweep values, "
