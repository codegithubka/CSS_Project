...
class WarmupStudyConfig:
    ...
    equilibration_window: int = (
        50  # FFT window size (needs to capture oscillation periods)
    )
...
def run_warmup_study(cfg: WarmupStudyConfig, logger: logging.Logger) -> Dict[int, Dict]:
    ...
    for L in cfg.grid_sizes:
        ...
        scaled_window = max(
            cfg.equilibration_window, int(cfg.equilibration_window * (L / 100))
        )
        ...
        for rep in range(cfg.n_replicates):
            ...
            if (rep + 1) % max(1, cfg.n_replicates // 5) == 0:
                logger.info(
                    f"  Replicate {rep+1}/{cfg.n_replicates}: "
        ...
        results[L] = {
            ...
            "mean_final_prey_density": float(
                np.mean(size_results["final_prey_density"])
            ),
            "mean_final_pred_density": float(
                np.mean(size_results["final_pred_density"])
            ),
        ...
        logger.info(f"\n  Summary for L={L}:")
        logger.info(
            f"    Time per step: {results[L]['mean_time_per_step']*1000:.2f} ± "
            f"{results[L]['std_time_per_step']*1000:.2f} ms"
        )
        logger.info(
            f"    Equilibration steps: {results[L]['mean_eq_steps']:.0f} ± "
            f"{results[L]['std_eq_steps']:.0f}"
        )
        logger.info(
            f"    Total warmup time: {results[L]['mean_total_warmup_time']:.2f} s"
        )
...
Examples:
  ...
  %(prog)s --output results/warmup_analysis/  # Custom output directory
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("results/warmup_study"),
        help="Output directory (default: results/warmup_study)",
    )
    parser.add_argument(
        "--dpi", type=int, default=150, help="Plot resolution (default: 150)"
    )
    parser.add_argument(
        "--prey-birth", type=float, default=0.22, help="Prey birth rate (default: 0.22)"
    )
    parser.add_argument(
        "--prey-death", type=float, default=0.04, help="Prey death rate (default: 0.04)"
    )
    parser.add_argument(
        "--diagnostic",
        action="store_true",
    ...
    if args.diagnostic:
        ...
        logger.info(
            "Adjust parameters based on the plots, then run without --diagnostic"
        )
        return
    ...
    if len(sizes) >= 2:
        ...
        logger.info(
            f"Equilibration steps scaling: t_eq ~ L^{z_eq:.2f} (R² = {r_eq**2:.3f})"
        )
        logger.info(
            f"Total warmup time scaling: T_warmup ~ L^{z_total:.2f} (R² = {r_total**2:.3f})"
        )
        logger.info(f"\nInterpretation:")
        ...
        if z_eq > 1.5:
            ...
            logger.info(
                f"  critical slowing down. Consider longer warmup for large grids."
            )
