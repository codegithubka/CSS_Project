...
class Config:
    ...
    warmup_steps: int = 200 * (
        default_grid / 100
    )  # FIXME: Steps to run before measuring
    ...
    def estimate_runtime(self, n_cores: int = 32) -> str:
        ...
        for L in self.fss_grid_sizes:
            ...
            l_warmup_scale = (
                L / self.default_grid
            )  # Time also scales with warmup duration
        ...
        n_total = (
            n_sweep + n_sens + sum(self.fss_replicates for _ in self.fss_grid_sizes)
        )
...
def run_single_simulation(
    ...
    for step in range(cfg.measurement_steps):
        ...
        if step >= cfg.cluster_interval and sample_counter < cfg.cluster_samples:
            if prey >= min_count and pred >= (min_count // 4):
                ...
                prey_perc, _, prey_perc_size, _ = get_percolating_cluster_fast(
                    model.grid, 1
                )
                pred_perc, _, pred_perc_size, _ = get_percolating_cluster_fast(
                    model.grid, 2
                )
                ...
                if compute_pcf:
                    ...
                    pcf_data = compute_all_pcfs_fast(
                        model.grid, max_dist, cfg.pcf_n_bins
                    )
            ...
            sample_counter += (
                1  # <-- FIX 3: Move outside the min_count check (was missing)
            )
    ...
    result = {
        ...
        "prey_largest_fraction_mean": (
            float(np.mean(prey_largest_fractions)) if prey_largest_fractions else np.nan
        ),
        "prey_largest_fraction_std": (
            float(np.std(prey_largest_fractions)) if prey_largest_fractions else np.nan
        ),
        "pred_largest_fraction_mean": (
            float(np.mean(pred_largest_fractions)) if pred_largest_fractions else np.nan
        ),
        # Percolation probability
        "prey_percolation_prob": (
            float(np.mean(prey_percolates)) if prey_percolates else np.nan
        ),
        "pred_percolation_prob": (
            float(np.mean(pred_percolates)) if pred_percolates else np.nan
        ),
    }
    ...
    if len(pcf_samples["prey_prey"]) > 0:
        ...
        if np.any(short_dist_mask):
            ...
            result["prey_clustering_index"] = float(
                np.mean(pcf_rr_mean[short_dist_mask])
            )
            result["pred_clustering_index"] = float(
                np.mean(pcf_cc_mean[short_dist_mask])
            )
    else:
...
def run_single_simulation_fss(
    ...
    result = {
        ...
        "prey_percolation_prob": (
            float(np.mean(prey_percolates)) if prey_percolates else np.nan # FIXME: Will not percolate
        ),
    }
...
def run_sensitivity(
    ...
    results = Parallel(n_jobs=cfg.n_jobs, verbose=0)(
        delayed(run_single_simulation)(
            pb, pd, gs, seed, evo, cfg, evolve_sd=sd, compute_pcf=True
        )
        for pb, pd, gs, seed, evo, sd in tqdm(
            jobs, desc="Sensitivity Progress", mininterval=10
        )
    )
...
def run_fss(cfg: Config, output_dir: Path, logger: logging.Logger) -> List[Dict]:
    ...
    tau_vals = [
        r["prey_tau"] for r in test_results if not np.isnan(r.get("prey_tau", np.nan))
    ]
    if tau_vals:
...
def run_debug_mode(cfg: Config, logger: logging.Logger):
    ...
    model.evolve(
        "prey_death", sd=cfg.evolve_sd, min_val=cfg.evolve_min, max_val=cfg.evolve_max
    )
...
def main():
    ...
    logger.info(
        f"Directed hunting: {'ENABLED' if cfg.directed_hunting else 'DISABLED'}"
    )
