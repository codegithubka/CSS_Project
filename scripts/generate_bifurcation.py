...
def generate_bifurcation_data(
    ...
    logging.info(
        f"  Sweep: {n_sweep} points from {prey_death_min:.3f} to {prey_death_max:.3f}"
    )
    ...
    try:
        ...
        for idx, rep, prey_pop, pred_pop in tqdm(
            parallel_results, total=len(jobs), desc="Bifurcation"
        ):
...
Examples:
  ...
  %(prog)s --prey-death-min 0.02 --prey-death-max 0.08  # Focus on critical region
    ...
    parser.add_argument(
        "--n-sweep", type=int, default=25, help="Number of sweep points (default: 25)"
    )
    parser.add_argument(
        "--n-replicates",
        type=int,
    ...
    # Grid and timing
    parser.add_argument(
        "--grid-size", type=int, default=100, help="Grid size (default: 100)"
    )
    parser.add_argument(
        "--warmup-steps",
        type=int,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("results/bifurcation"),
    ...
    logging.info(
        f"Prey population range: {prey_results.min():.0f} - {prey_results.max():.0f}"
    )
    logging.info(f"Prey mean population: {prey_results.mean():.1f}")
    logging.info(
        f"Predator population range: {predator_results.min():.0f} - {predator_results.max():.0f}"
    )
    logging.info(f"Predator mean population: {predator_results.mean():.1f}")
    logging.info(f"")
    logging.info(
        f"To plot: python scripts/analysis.py {args.output} --bifurcation-only"
    )
