...
def create_test_grid(size: int, seed: int = 42) -> Tuple[np.ndarray, np.ndarray]:
    ...
    np.random.seed(seed)
    grid = np.random.choice([0, 1, 2], (size, size), p=[0.55, 0.30, 0.15]).astype(
        np.int32
    )
...
def print_result(
    ...
    if speedup and baseline_name:
        print(
            f"  {name:<35} {time_ms:>8.2f} ± {std_ms:>5.2f} ms  "
...
def pp_kernel_python(
    grid: np.ndarray,
    prey_death: np.ndarray,
    prey_birth: float,
    prey_death_rate: float,
    pred_birth: float,
    pred_death: float,
    n_steps: int = 1,
) -> np.ndarray:
...
def benchmark_kernel(
    grid_sizes: List[int] = [50, 100, 150],
    n_steps: int = 100,
    ...
    for size in grid_sizes:
        ...
        if NUMBA_AVAILABLE:
            ...
            results.append(
                BenchmarkResult("kernel_numba_random", size, mean_ms, n_steps, "numba")
            )
            numba_time = mean_ms
        ...
        if NUMBA_AVAILABLE:
            ...
            results.append(
                BenchmarkResult(
                    "kernel_numba_directed", size, mean_ms, n_steps, "numba"
                )
            )
        ...
        if include_baseline and size <= 50:
            ...
            results.append(
                BenchmarkResult("kernel_python", size, mean_ms, n_steps, "python")
            )
...
def benchmark_pcf(
    grid_sizes: List[int] = [50, 100, 150], include_baseline: bool = True
) -> List[BenchmarkResult]:
    ...
    for size in grid_sizes:
        ...
        if NUMBA_AVAILABLE:
            ...
            results.append(
                BenchmarkResult(
                    "pcf_celllist", size, mean_ms, 1, "numba", {"n_prey": n_prey}
                )
            )
        ...
        if include_baseline and size <= 75:
            ...
            print_result(
                "Brute-force PCF (Python)", mean_ms, std_ms, speedup, "Cell-list"
            )
            results.append(
                BenchmarkResult(
                    "pcf_bruteforce", size, mean_ms, 1, "python", {"n_prey": n_prey}
                )
            )
...
def benchmark_clusters(
    grid_sizes: List[int] = [50, 100, 200], include_baseline: bool = True
) -> List[BenchmarkResult]:
...
def benchmark_full_simulation(
    grid_sizes: List[int] = [50, 100], n_steps: int = 200
) -> List[BenchmarkResult]:
    ...
    for size in grid_sizes:
        ...
        if NUMBA_AVAILABLE:
            ...
            results.append(
                BenchmarkResult("full_random", size, mean_ms, n_steps, "numba")
            )

            # Directed hunting
            mean_ms_dir, std_ms_dir = timeit(
                lambda: run_full_sim(True), n_runs=3, warmup=1
            )
            print_result("Full sim (directed)", mean_ms_dir, std_ms_dir)
            results.append(
                BenchmarkResult("full_directed", size, mean_ms_dir, n_steps, "numba")
            )
...
def benchmark_scaling(max_size: int = 200, n_points: int = 6) -> List[BenchmarkResult]:
    ...
    for size in sizes:
        if NUMBA_AVAILABLE:
            ...
            results.append(
                BenchmarkResult("scaling_kernel", size, kernel_ms, 50, "numba")
            )
...
def print_summary(all_results: List[BenchmarkResult]):
    ...
    kernel_python = [
        r for r in all_results if r.name == "kernel_python" and r.grid_size == 50
    ]
    kernel_numba = [
        r for r in all_results if r.name == "kernel_numba_random" and r.grid_size == 50
    ]
    ...
    # PCF speedup
    pcf_brute = [
        r for r in all_results if r.name == "pcf_bruteforce" and r.grid_size == 50
    ]
    pcf_cell = [
        r for r in all_results if r.name == "pcf_celllist" and r.grid_size == 50
    ]
    ...
    # Cluster speedup
    cluster_scipy = [
        r for r in all_results if r.name == "cluster_scipy" and r.grid_size == 100
    ]
    cluster_numba = [
        r for r in all_results if r.name == "cluster_numba" and r.grid_size == 100
    ]
    ...
    kernel_random = [
        r for r in all_results if r.name == "kernel_numba_random" and r.grid_size == 100
    ]
    kernel_directed = [
        r
        for r in all_results
        if r.name == "kernel_numba_directed" and r.grid_size == 100
    ]
    ...
    # Throughput
    full_results = [
        r for r in all_results if r.name == "full_random" and r.grid_size == 100
    ]
    if full_results:
        steps_per_sec = full_results[0].iterations / (full_results[0].time_ms / 1000)
        print(
            f"  • Simulation throughput:    {steps_per_sec:>6.0f} steps/sec (100x100)"
        )
...
def export_results(results: List[BenchmarkResult], filepath: str):
    ...
    with open(filepath, "w") as f:
        ...
        for r in results:
            f.write(
                f"{r.name},{r.grid_size},{r.time_ms:.3f},{r.iterations},"
...
def generate_plots(results: List[BenchmarkResult], filepath: str):
    ...
    for variant, label, color in [
        ...
        if data:
            sizes, times = zip(*sorted(data))
            ax.plot(
                sizes, times, "o-", label=label, color=color, linewidth=2, markersize=8
            )
    ax.set_xlabel("Grid Size")
    ax.set_ylabel("Time (ms)")
    ...
    ax = axes[1, 0]
    kernel_data = [
        (r.grid_size, r.time_ms) for r in results if r.name == "scaling_kernel"
    ]
    ...
    if kernel_data:
        ...
        ax.plot(
            sizes, times, "o-", label="Kernel (50 steps)", linewidth=2, markersize=8
        )
    if pcf_data:
    ...
    kernel_nb = next(
        (
            r.time_ms
            for r in results
            if r.name == "kernel_numba_random" and r.grid_size == 50
        ),
        None,
    )
    ...
    pcf_bf = next(
        (
            r.time_ms
            for r in results
            if r.name == "pcf_bruteforce" and r.grid_size == 50
        ),
        None,
    )
    pcf_cl = next(
        (r.time_ms for r in results if r.name == "pcf_celllist" and r.grid_size == 50),
        None,
    )
...
def main():
    ...
    parser.add_argument(
        "--no-baseline", action="store_true", help="Skip baseline comparisons"
    )
    ...
    if args.quick:
        grid_sizes = [50, 100]
        all_results.extend(
            benchmark_kernel(grid_sizes, n_steps=50, include_baseline=include_baseline)
        )
        all_results.extend(benchmark_pcf([50], include_baseline=include_baseline))
    else:
        all_results.extend(
            benchmark_kernel(
                [50, 100, 150], n_steps=100, include_baseline=include_baseline
            )
        )
        all_results.extend(
            benchmark_pcf([50, 75, 100], include_baseline=include_baseline)
        )
        all_results.extend(
            benchmark_clusters([50, 100, 150], include_baseline=include_baseline)
        )
